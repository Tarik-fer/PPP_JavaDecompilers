...
implements Serializable {
    ...
    public String[] scoreStrings = new String[this.scores.length];
    public String[] multiplayerScoreStrings = new String[this.multiplayerScores.length];
    ...
    public boolean checkNewHighScore() {
        for (int i = 0; i < this.scores.length; ++i) {
            if (this.score <= this.scores[i]) continue;
            int min = -1;
            for (int j = 0; j < this.scores.length; ++j) {
                if (min <= this.scores[j]) continue;
                min = this.scores[j];
            }
            int c = 0;
            for (int j = 0; j < this.scores.length; ++j) {
                if (this.scores[j] != min) continue;
                c = j;
                break;
            }
    ...
    public boolean checkNewMultiplayerHighScore() {
        for (int i = 0; i < this.multiplayerScores.length; ++i) {
            if (this.score <= this.multiplayerScores[i]) continue;
            int min = -1;
            for (int j = 0; j < this.multiplayerScores.length; ++j) {
                if (min <= this.multiplayerScores[j]) continue;
                min = this.multiplayerScores[j];
            }
            int c = 0;
            for (int j = 0; j < this.multiplayerScores.length; ++j) {
                if (this.multiplayerScores[j] != min) continue;
                c = j;
                break;
            }
