...
public abstract class DirectoryWalker
{
    ...
    private void walk(final File directory, final int depth, final Collection results) throws IOException {
        ...
        if (this.handleDirectory(directory, depth, results)) {
            ...
            if (this.depthLimit < 0 || childDepth <= this.depthLimit) {
                this.checkIfCancelled(directory, depth, results);
                final File[] childFiles = (this.filter == null) ? directory.listFiles() : directory.listFiles(this.filter);
                if (childFiles == null) {
                    this.handleRestricted(directory, childDepth, results);
                }
                else {
                    for (int i = 0; i < childFiles.length; ++i) {
                        final File childFile = childFiles[i];
    ...
    protected final void checkIfCancelled(final File file, final int depth, final Collection results) throws IOException {
    ...
    protected boolean handleDirectory(final File directory, final int depth, final Collection results) throws IOException {
        return true;
    }
    ...
    protected void handleRestricted(final File directory, final int depth, final Collection results) throws IOException {
    }
