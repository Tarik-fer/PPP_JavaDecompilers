...
public class FileSystemUtils
{
    ...
    public static long freeSpace(final String path) throws IOException {
        return FileSystemUtils.INSTANCE.freeSpaceOS(path, FileSystemUtils.OS, false);
    }
    
    public static long freeSpaceKb(final String path) throws IOException {
        return FileSystemUtils.INSTANCE.freeSpaceOS(path, FileSystemUtils.OS, true);
    }
    
    long freeSpaceOS(final String path, final int os, final boolean kb) throws IOException {
        if (path == null) {
    ...
    long freeSpaceWindows(String path) throws IOException {
        ...
        final List lines = this.performCommand(cmdAttribs, Integer.MAX_VALUE);
    ...
    long parseDir(final String line, final String path) throws IOException {
        ...
        final StringBuffer buf = new StringBuffer(line.substring(bytesStart, bytesEnd));
    ...
    long freeSpaceUnix(String path, final boolean kb, final boolean posix) throws IOException {
        if (path.length() == 0) {
            throw new IllegalArgumentException("Path must not be empty");
        }
        path = FilenameUtils.normalize(path);
        ...
        final List lines = this.performCommand(cmdAttribs, 3);
        if (lines.size() < 2) {
    ...
    List performCommand(final String[] cmdAttribs, final int max) throws IOException {
        final List lines = new ArrayList(20);
        Process proc = null;
        InputStream in = null;
